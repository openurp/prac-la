...
[@b.grid items=laVolunteers var="laVolunteer"]
  [@b.gridbar]
    bar.addItem("${b.text("action.export")}",
      action.exportData("std.user.code:学号,std.user.name:姓名,std.state.department.name:学院,std.state.major.name:专业,mobile:联系方式,enrolledRank:录取志愿,enrolledOption.corporation.name:录取单位,gpa:平均绩点,updatedAt:报名时间",null,'fileName=报名信息'));
  [/@]
  [@b.row]
    [@b.boxcol /]
      [@b.col property="std.user.code" title="学号" width="10%"/]
      [@b.col property="std.user.name" title="姓名" width="9%"]<span title="${laVolunteer.updatedAt?string('MM-dd HH:mm')}报名,${laVolunteer.mobile}">${laVolunteer.std.user.name}</span>[/@]
      [@b.col property="std.state.grade" title="年级" width="7%"/]
      [@b.col property="std.state.department.name" title="院系" width="16%"/]
      [@b.col property="std.state.major.name" title="专业" width="15%"]${(laTaker.std.state.major.name)!}[/@]
