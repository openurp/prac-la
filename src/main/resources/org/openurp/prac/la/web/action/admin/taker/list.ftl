...
[@b.grid items=laTakers var="laTaker"]
  [@b.gridbar]
    ...
    bar.addItem("${b.text("action.export")}",
      action.exportData("volunteer.std.code:学号,volunteer.std.name:姓名,volunteer.std.state.department.name:学院,volunteer.std.state.major.name:专业,volunteer.mobile:联系方式,rank:志愿顺序,option.corporation.name:单位,volunteer.gpa:平均绩点,updatedAt:报名时间",null,'fileName=志愿信息'));
  [/@]
  [@b.row]
    [@b.boxcol /]
